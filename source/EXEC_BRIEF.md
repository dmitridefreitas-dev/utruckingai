...
- **Built to stay up** — a caching layer keeps quotes and lookups working through a transient Google Sheets outage, learned AI item-matches are cached so repeats are instant and free, and a **54-test automated suite runs on every code change** so nothing quietly breaks.
